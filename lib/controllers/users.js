...
var userDB = nano.db.use(config.couchdb.users);
  ...
  userDB.head(friendName, function (error, _, headers) {
    ...
    if(headers && headers['status-code'] === 200) {
      userDB.get(username, function (error, user) {
        ...
        for(var friendCtr = 0; friendCtr < user.friends.length; friendCtr++) {
          if(user.friends[friendCtr] === friendName) {
            return res.json(400, {message: 'Already friends with user.'});
          }
        }
        ...
        userDB.insert(user, username, function (error, body) {
          ...
          if(body.ok) {
            // TODO When a user adds a friend to their list, alert the added user.
...
// Use gravatar or take the new file, send it to some storage, update DB with url to file, then pass success
...
exports.changeEmail = function(req, res) {
  ...
  userDB.get(username, function (error, user) {
    ...
    user.email = newEmail;
    userDB.insert(user, username, function (error) {
      if(error) {
        console.log('userDB.insert');
        console.log(error);
        return res.json(500, {message: 'Error changing email, please try again later.'});
...
exports.receiveSignal = function(req, res) {
  if(!req.session.user_id) {
    return res.json(401, {message: 'Please sign in.'});
  }
  ...
  var username = req.session.user_id;
  if(validator.isNull(latitude) || validator.isNull(longitude)) {
    return res.json(400, {message: 'Missing coordinates.'});
  }
  var coordinates = {latitude: latitude, longitude: longitude};
  gm.reverseGeocode(coordinates.latitude + ',' + coordinates.longitude, function (err, locationData){
    if(err) {
      console.log(err);
      return res.json(500, {message: 'Could not update location, please try again later.'});
    }
    var zipCode = null;
    for(var componentCounter = 0; componentCounter < locationData.results[0].address_components.length; componentCounter++) {
      for(var typeCounter = 0; typeCounter < locationData.results[0].address_components[componentCounter].types.length; typeCounter++) {
        if(locationData.results[0].address_components[componentCounter].types[typeCounter] === 'postal_code') {
          zipCode = locationData.results[0].address_components[componentCounter].long_name;
        }
      }
    }
    if(!zipCode) {
