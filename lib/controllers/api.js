...
exports.getCloseUsers = function(req, res) {
  ...
  userDB.get(username, function (error, user) {
    ...
    userDB.view('users', 'by_zipcode', {reduce: false, startkey: zipCode, endkey: zipCode + '\u9999'}, function (error, body) {
      if (error) {
        console.log(error);
        return res.json({users: [], message: 'No results found.'});
      }
      ...
      for(var userCounter = 0; userCounter < body.rows.length; userCounter++) {
        ...
        var distanceObj = _distanceCalculator(coords.latitude, coords.longitude, tempCoordinates.latitude, tempCoordinates.longitude);
        if(distanceObj.meters <= 1000 && body.rows[userCounter].value._id !== username) {
          body.rows[userCounter].value.distance = distanceObj;
          delete body.rows[userCounter].value.password;
          delete body.rows[userCounter].value.zipcode;
          delete body.rows[userCounter].value.isOwnProfile;
          delete body.rows[userCounter].value.email;
          delete body.rows[userCounter].value._rev;
          delete body.rows[userCounter].value.friends;
          closeUsers.push(body.rows[userCounter].value);
        }
      }
      return res.json({users: closeUsers});
    });
  });
};